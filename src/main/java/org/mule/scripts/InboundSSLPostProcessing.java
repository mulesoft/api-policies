 ...
 */
public class InboundSSLPostProcessing implements Scriptable {

	private static final String HTTP_LISTENER = "http:listener";
	private static final String HTTP_CONFIG = "http:listener-config";	
	private static final String HTTPS_TLS_KEY_STORE = "tls:key-store";
	...
	private static final String HTTPS_CONFIG_PATH = "/https-inbound-config.xml";
	private static final String HTTPS_CONNECTOR_PATH = "/https-inbound.xml";
	...
	protected static Logger LOGGER = LogManager.getLogger(InboundSSLPostProcessing.class);
	...
	public void process(Map<String, String> input) throws Exception {
		...
		final String aliasPassword = input.get(Constants.KEY_ALIAS_PASSWORD).toString();
		...
		if (aliasPassword == null){
			throw new IllegalArgumentException("Alias password is not provided.");
		}
		...
		final Document doc = injectHTTPS(docBuilder.parse(filepath), docBuilder, storePassword, aliasPassword, certFile.getName(), 
	 ...
	 */
	private Document injectHTTPS(Document doc, DocumentBuilder docBuilder, String storePassword, String aliasPassword, String certPath, String root) throws SAXException, IOException{
		...
		final NodeList httpReqConfig = doc.getElementsByTagName(HTTP_CONFIG);
		...
		final String httpConfigPath = HTTPS_CONFIG_PATH;
		final Node httpsConnectorNode = doc.importNode(docBuilder.parse(getClass().getResourceAsStream(httpConfigPath)).getDocumentElement(), true);
		final Element tlsNode = (Element) ((Element) httpsConnectorNode).getElementsByTagName(HTTPS_TLS_KEY_STORE).item(0);
		tlsNode.setAttribute("path", certPath);
		tlsNode.setAttribute("password", storePassword);
		tlsNode.setAttribute("keyPassword", aliasPassword);
		fis.close();
		doc.getDocumentElement().appendChild(httpsConnectorNode);
		
				
		// replace HTTP request component with HTTPS
		final NodeList flows = doc.getElementsByTagName(HTTP_LISTENER);
		if (flows.getLength() > 0){
			final Node httpsNode = doc.importNode(docBuilder.parse(getClass().getResourceAsStream(HTTPS_CONNECTOR_PATH)).getDocumentElement(), true);
			final Node httpInboundNode = flows.item(0); 
			httpInboundNode.getParentNode().replaceChild(httpsNode, httpInboundNode);
		}
		if (flows.getLength() > 1){
			final Node httpsNode1 = doc.importNode(docBuilder.parse(getClass().getResourceAsStream(HTTPS_CONNECTOR_PATH)).getDocumentElement(), true);
			final Node httpConsoleNode = flows.item(1); 			
			((Element) httpsNode1).setAttribute("path", "![p['console.path']]");
			httpConsoleNode.getParentNode().replaceChild(httpsNode1, httpConsoleNode);
